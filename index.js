...
const { isLabeledStatement } = require("@babel/types");
...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: the nested function can still access the internal function because const is block scoped and nestedFunction is in the block of myFunction so it can access internal
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    let result = [];
    arr.forEach(element => result.push(`name: ${element.animal_name}, scientific: ${element.scientific_name}`));
    return result;
  }
  ...
  */
  ...
  function USApop(arr){
    return arr.reduce((total, element) => total+=element.population, 0);
  }
